...
namespace TaskManagement.Services.EF.Repositories
{
    public class TaskRepository : ITaskRepository
    {
        ...
        {
            return _context.TasksExecution.FirstOrDefault(task => task.TaskId == taskId);
        }

        public TaskExecution GetTaskExecution(Guid taskExecutionId)
        {
            return _context.TasksExecution.Include(x => x.Task).FirstOrDefault(task => task.Id == taskExecutionId);
        }
